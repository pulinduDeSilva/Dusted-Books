...
export default function SellBook() {
  ...
  /* ── Book helpers ── */
  const updateBook = <K extends keyof BookEntry>(
    i: number,
    key: K,
    val: BookEntry[K],
  ) =>
    setBooks((prev) =>
      prev.map((b, idx) => (idx === i ? { ...b, [key]: val } : b)),
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < books.length; i++) {
      const b = books[i];
      if (!b.bookTitle.trim())
        return setSubmitError(`Book ${i + 1}: title is required.`);
      if (!b.authorName.trim())
        return setSubmitError(`Book ${i + 1}: author name is required.`);
      if (mode === "individual") {
  ...
  const labelCls =
    "block mb-1.5 text-xs font-semibold uppercase tracking-wider text-stone-500 dark:text-gray-400";
      ...
      <div className="min-h-screen bg-[#fcfaf8] dark:bg-gray-950 overflow-x-hidden w-full max-w-full">
        ...
        <main className="mx-auto max-w-lg px-4 pb-20 pt-36 text-center">
          ...
          <div className="relative mx-auto mb-8 flex h-28 w-28 items-center justify-center">
            ...
            <svg
              className="relative h-12 w-12 text-emerald-600 dark:text-emerald-400"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2.5}
          ...
          <p className="mb-10 text-sm text-stone-400 dark:text-gray-500">
            ...
            <span className="font-semibold text-stone-600 dark:text-gray-300">
              {contact}
            </span>{" "}
            to arrange pickup.
          </p>
            ...
            <div className="flex flex-wrap gap-2">
              {books
                ...
                .map((b, i) => (
                  <span
                    key={i}
                    className="rounded-full bg-amber-50 px-3 py-1 text-xs font-medium text-amber-800 ring-1 ring-amber-200 dark:bg-amber-900/20 dark:text-amber-300 dark:ring-amber-700/40"
                  >
                    {b.bookTitle}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex items-center gap-3 border-b border-stone-100 px-6 py-4 dark:border-gray-700/60">
              <div>
                ...
                <p className="text-xs text-stone-400 dark:text-gray-500">
                  How would you like to price your books?
                </p>
              </div>
            </div>
            <div className="p-5">
                ...
                {[
                  {
                    ...
                    desc: "Each book has its own price",
                  },
                  {
                    value: "series" as const,
                    title: "Series / Bundle",
                    desc: "All books at one price",
                  },
                ].map((m) => (
                  <button
                    key={m.value}
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-semibold leading-tight ${mode === m.value ? "text-amber-800 dark:text-amber-300" : "text-stone-800 dark:text-gray-200"}`}
                    >
                      {m.title}
                    </span>
                    <span className="text-xs text-stone-500 dark:text-gray-400 leading-snug">
                      {m.desc}
                    </span>
                  </button>
                ))}
            ...
            <div className="grid gap-4 p-5 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                ...
                <div className="relative">
                  <svg
                    className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400 dark:text-gray-500"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
              ...
              <div className="flex items-center gap-3 border-b border-amber-200/60 px-6 py-4 dark:border-amber-700/30">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700/70 dark:text-amber-400/70">
                    Set a single price for the entire collection
                  </p>
                </div>
              </div>
              <div className="p-5">
                <label
                  htmlFor="series-price"
                  className="block mb-1.5 text-xs font-semibold uppercase tracking-wider text-amber-700 dark:text-amber-400"
                >
                  Total asking price (LKR){" "}
                ...
                <div className="relative">
                  ...
                  />
                  {seriesPrice &&
                    !isNaN(Number(seriesPrice)) &&
                    Number(seriesPrice) > 0 && (
                ...
                {seriesPrice &&
                  ...
                  Number(seriesPrice) > 0 && (
                    <p className="mt-2 text-xs text-amber-700/70 dark:text-amber-400/70">
                      {formatLKR(seriesPrice)} for all {books.length} book
                      {books.length > 1 ? "s" : ""}
                    </p>
                  )}
              </div>
          ...
          <div className="rounded-2xl border border-stone-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900 overflow-hidden">
            <div className="flex items-center justify-between border-b border-stone-100 px-5 py-4 dark:border-gray-700/60">
              <div className="flex items-center gap-3">
                <div>
                  ...
                  <p className="text-xs text-stone-400 dark:text-gray-500">
                    Add the books you'd like to sell
                  </p>
                </div>
              </div>
              <button
            ...
            <ul className="flex flex-col divide-y divide-stone-100 dark:divide-gray-700/60">
              {books.map((book, i) => (
                <li
                  key={i}
                  className="group px-5 py-5 transition-colors hover:bg-stone-50/50 dark:hover:bg-gray-800/20"
                >
                  {/* Row header */}
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {book.previewUrl ? (
                        ...
                        <div className="flex h-24 w-16 flex-col items-center justify-center gap-1.5 rounded-xl border-2 border-dashed border-stone-200 bg-stone-50 transition hover:border-amber-400 hover:bg-amber-50/50 dark:border-gray-700 dark:bg-gray-800 dark:hover:border-amber-500 dark:hover:bg-amber-900/10">
                          <svg
                            className="h-5 w-5 text-stone-300 dark:text-gray-600"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            strokeWidth={1.5}
                          ...
                          <span className="text-[9px] font-medium text-stone-300 dark:text-gray-600">
                            Photo
                          </span>
                        </div>
                      )}
                    ...
                    <div className="flex flex-1 flex-col gap-2.5 min-w-0">
                      <input
                        ...
                        onChange={(e) =>
                          updateBook(i, "bookTitle", e.target.value)
                        }
                        className={inputCls}
                      />
                      <input
                        ...
                        onChange={(e) =>
                          updateBook(i, "authorName", e.target.value)
                        }
                        className={inputCls}
                      />
                      {mode === "individual" && (
                        <div className="relative">
                          ...
                          <input
                            ...
                            value={book.price}
                            onChange={(e) =>
                              updateBook(i, "price", e.target.value)
                            }
                            className={`${inputCls} pl-12`}
                          />
                          {book.price &&
                ...
                {books
                  ...
                  .map((b, i) => (
                    ...
                    >
                      ...
                      {mode === "individual" && b.price && (
                        <span className="ml-1.5 font-bold text-amber-700 dark:text-amber-400">
                          {formatLKR(b.price)}
                        </span>
                      )}
                    </span>
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="text-xs text-stone-400 dark:text-gray-500">
              ...
              {mode === "series" ? "Bundle pricing" : "Individual pricing"}
            </p>
          </div>
        </form>
      </main>
