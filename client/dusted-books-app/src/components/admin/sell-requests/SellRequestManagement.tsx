...
> = {
  ...
  denied: {
    ...
    dot: "bg-red-500",
    badge:
      "bg-red-50 text-red-600 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-700/50",
...
const StatCard = ({
  ...
  <div
    className={`rounded-xl border bg-white px-4 py-3 shadow-sm dark:bg-gray-900 ${color}`}
  >
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setError("Could not load sell requests.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard
          ...
          color="border-amber-200 dark:border-amber-700/40"
        />
        <StatCard
          label="Approved"
          value={counts.approved + counts.pickup_confirmed}
          color="border-emerald-200 dark:border-emerald-700/40"
        />
        <StatCard
          label="Completed"
          value={counts.trade_accepted}
          color="border-blue-200 dark:border-blue-700/40"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-16 text-center dark:border-gray-700 dark:bg-gray-900">
          ...
          <p className="text-sm font-medium text-gray-400 dark:text-gray-500">
            No{" "}
            {filter === "all" ? "" : filterConfig[filter].label.toLowerCase()}{" "}
            requests found.
          </p>
        </div>
      ) : (
        <ul className="flex flex-col gap-4">
          {filtered.map((req) => {
            ...
            const totalIndividual = req.books.reduce(
              (s, b) => s + (b.price ?? 0),
              0,
            );
            const displayPrice =
              req.sellType === "series" ? req.seriesPrice : totalIndividual;
            const priceLabel =
              req.sellType === "series"
                ? "Bundle price"
                : req.books.length > 1
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-2.5">
                      {req.books.map((b, i) => (
                        <span
                          key={i}
                          className="rounded-full bg-gray-50 px-2.5 py-0.5 text-xs text-gray-700 ring-1 ring-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:ring-gray-700"
                        >
                ...
                {req.books.some((b) => b.photoUrl) && (
                  <>
                    <button
                      type="button"
                      onClick={() =>
                        setExpanded((p) => ({ ...p, [req._id]: !p[req._id] }))
                      }
                    ...
                    >
                      <span className="flex items-center gap-1.5">
                        ...
                        {isExpanded ? "Hide" : "View"} book photos (
                        {req.books.filter((b) => b.photoUrl).length})
                      </span>
                      <svg
                        ...
                        {req.books
                          ...
                          .map((b, i) => (
                            ...
                            >
                              ...
                              <p className="max-w-[80px] truncate text-center text-[10px] font-medium text-gray-500 dark:text-gray-400">
                                {b.bookTitle}
                              </p>
                              {req.sellType === "individual" &&
                                b.price != null && (
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      {showNotes && (
                        <DenyForm
                          ...
                          onNotes={(v) =>
                            setAdminNotes((p) => ({ ...p, [req._id]: v }))
                          }
                          onConfirm={() => handleAction(req._id, "denied")}
                          onCancel={() =>
                            setNotesOpen((p) => ({ ...p, [req._id]: false }))
                          }
                  ...
                  {req.status === "approved" && (
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center gap-2 rounded-xl bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300">
                        ...
                        Approved — contact the customer and confirm pickup when
                        ready.
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <button
                          onClick={() =>
                            handleAction(req._id, "pickup_confirmed")
                          }
                          disabled={isLoading}
                  ...
                  {req.status === "pickup_confirmed" && (
                    <div className="flex flex-col gap-3">
                      <div className="flex items-center gap-2 rounded-xl bg-blue-50 px-4 py-3 text-sm font-medium text-blue-700 dark:bg-blue-900/20 dark:text-blue-300">
                        ...
                        Pickup confirmed — accept or deny the trade once the
                        book is received.
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <button
                          onClick={() =>
                            handleAction(req._id, "trade_accepted")
                          }
                          disabled={isLoading}
                      ...
                      {showNotes && (
                        <DenyForm
                          ...
                          onConfirm={() =>
                            handleAction(req._id, "trade_denied")
                          }
                          onCancel={() =>
                            setNotesOpen((p) => ({ ...p, [req._id]: false }))
                          }
                  ...
                  {req.status === "trade_accepted" && (
                    <div className="flex items-center gap-3 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 px-4 py-3.5 ring-1 ring-emerald-200 dark:from-emerald-900/20 dark:to-teal-900/10 dark:ring-emerald-700/40">
                      ...
                      <p className="text-sm font-semibold text-emerald-700 dark:text-emerald-300">
                        Trade accepted — transaction complete.
                      </p>
                    </div>
                  )}

                  {(req.status === "denied" ||
                    req.status === "trade_denied") && (
...
function DenyForm({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-700/50 dark:bg-red-900/20">
      <div>
        <label className="mb-1.5 block text-xs font-semibold text-red-700 dark:text-red-300">
          Reason (optional)
        </label>
        <textarea
          rows={2}
